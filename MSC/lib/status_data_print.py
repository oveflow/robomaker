...
class status_parser:
    def __init__(self):
        ...
        self.dict_data_launcher_before_login={            
            "0x0001" : "대기상태"
        }

        #launcher_status(0x02)
        self.dict_data_launcher_after_login={                      
        ...
        self.dict_data_simulator_status_lobby={            
            "0x0001" : "대기상태",
            "0x0002" : "맵/차량 선택 안됨",
            "0x0003" : "로딩중"
        }
        ...
        self.dict_data_simulator_status_play={                        
            "0x0001" : "대기 상태(플레이 상태)",
            "0x0002" : "시뮬레이션 정지 상태",
            "0x0003" : "로딩중",            
            ...
            if self.platform == Platform.LUANCHER:
                ...
                if self.dict_data_launcher_stage[self.stage] == Stage.BEFORE_LOGIN:
                    if int(self.is_wait,16) < int(self.status,16):
                        ...
                        print("Data_Status = {}".format(self.dict_data_launcher_before_login[self.status]))
                ...
                #After Login
                    ...
                    if int(self.is_wait,16) < int(self.status,16):
                        ...
                        print("Data_Status = {}".format(self.dict_data_launcher_after_login[self.status]))
            ##Simulator
            else:
                ...
                if self.stage == Stage.LOBBY:                
                    if int(self.is_wait,16) < int(self.status,16):
                        ...
                        print("Data_Status = {}".format(self.dict_data_simulator_status_lobby[self.status]))

                else:
                    if int(self.is_wait,16) < int(self.status,16):
                        print("Data_Status = Wait_status")
                    else:
                        print("Data_Status = {}".format(self.dict_data_simulator_status_play[self.status]))
